...
module.exports = function (context, input, cb) {
  ...
  const th = new TokenHandler(null, authUrl, storages, log, request)

  // TODO: clarify if that is correct
  if (strategy !== 'basic') cb(new Error('invalid login strategy'))

  login(th, userCredentials, (err, accessToken) => {
    ...
    th.deleteGuestTokens((err) => {
      if (err) return cb(err)
      cb(null, {userId: userCredentials.login})
    })
  })
}
...
function login (tokenHandler, userCredentials, cb) {
  tokenHandler.login(userCredentials.login, userCredentials.password, (err, accessToken) => {
    if (err) return cb(err)
    cb(null, accessToken)
  })
}
